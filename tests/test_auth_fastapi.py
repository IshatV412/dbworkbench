...
class TestGetCurrentUser:
    ...
    def test_valid_token(self, user):
        """TC_35 — Verify that a valid JWT token grants access."""
    ...
    def test_expired_token_raises_401(self, user):
        """TC_34 — Verify that an expired JWT token raises 401."""
    ...
    def test_invalid_token_raises_401(self):
        """TC_34 — Verify that an invalid token raises 401."""
        from fastapi import HTTPException
        from fastapi.security import HTTPAuthorizationCredentials
...
class TestProtectedEndpointsNoAuth:
    ...
    async def test_protected_endpoint_returns_401_without_token(self, method, path):
        """TC_73 — Verify that all protected endpoints return 401 without a JWT token."""
...
class TestAuthenticatedEndpoints:
    ...
    async def test_unauthorized_request_rejected(self):
        """TC_58 — Verify that an API request without a valid JWT token is rejected with 401."""
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app),
            base_url="http://testserver",
        ) as client:
    ...
    async def test_authenticated_query_execution(self, user, connection_profile, auth_headers):
        """TC_59 / TC_74 — Verify that an authenticated user can execute a query through the API."""
        mock_conn = MagicMock()
    ...
    async def test_query_via_full_api_stack(self, user, connection_profile, auth_headers):
        """TC_50 — Verify executing SQL through the full API stack returns correct output."""
        mock_conn = MagicMock()
        mock_cursor = MagicMock()
        mock_cursor.description = [("id",), ("name",)]
        mock_cursor.fetchall.return_value = [(1, "Alice"), (2, "Bob")]
