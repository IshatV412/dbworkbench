...
class TestAlterTableRealDB:
    ...
    def test_add_constraint(self):
        ...
        execute_steps(conn, inv.steps)
        # Verify constraint is gone — inserting negative should work now
        cur = conn.cursor()
    ...
    def test_drop_constraint(self):
        ...
        execute_steps(conn, inv.steps)
        # Constraint should be back — inserting negative should fail
        cur = conn.cursor()
    ...
    def test_set_not_null(self):
        ...
        execute_steps(conn, inv.steps)
        # NOT NULL removed — inserting NULL should work
        cur = conn.cursor()
    ...
    def test_drop_not_null(self):
        ...
        execute_steps(conn, inv.steps)
        # NOT NULL restored — inserting NULL should fail
        cur = conn.cursor()
    ...
    def test_set_default(self):
        ...
        execute_steps(conn, inv.steps)
        # Default removed — insert without qty should give NULL
        cur = conn.cursor()
...
class TestViewRealDB:
    ...
    def test_create_or_replace_view_adds_columns_limitation(self):
        """Known limitation: CREATE OR REPLACE VIEW that adds columns — the inverse
    ...
    def test_create_or_replace_view_narrowing_limitation(self):
        ...
        reversed with CREATE OR REPLACE — PG disallows dropping columns from a view."""
        conn = _test_conn()
        cur = conn.cursor()
        cur.execute("CREATE TABLE orders (id SERIAL PRIMARY KEY, item VARCHAR(50), qty INT)")
        ...
        # Inverse tries CREATE OR REPLACE back to original (fewer columns) — PG rejects
        with pytest.raises(psycopg2.errors.InvalidTableDefinition):
            execute_steps(conn, inv.steps)
        conn.rollback()
